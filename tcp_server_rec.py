...
def append_buffer(self, buffer):
    if os.path.isfile(self.filename):
        with wave.open(self.filename, 'ab') as wavfile:
            wavfile.setparams((1, self.bit_rate / 8, self.sample_rate, 0, 'NONE', 'NONE'))
            wavfile.writeframes(buffer)
            self.file_data = bytearray()
    else:
